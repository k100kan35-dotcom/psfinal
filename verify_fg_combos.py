#!/usr/bin/env python3
"""Try all temperature combinations to find best match."""
import sys
import numpy as np
...
print("Available temperatures:", temps)
print()
...
for r in range(1, len(temps) + 1):
    for combo in combinations(temps, r):
        ...
        try:
            ...
            g_mae = np.mean(np.abs(g_avg - exp_g) / np.where(exp_g > 0, exp_g, 1) * 100)
...
print("=" * 90)
print("ALL TEMPERATURE COMBINATIONS - Ranked by total MAE")
print("=" * 90)
print(f"{'Rank':>4s}  {'Temperatures':>30s}  {'f_MAE%':>8s}  {'g_MAE%':>8s}  {'Total%':>8s}")
print("-" * 90)
for i, r in enumerate(results):
    temps_str = ", ".join(f"{t:.1f}" for t in r['combo'])
    print(f"{i+1:4d}  {temps_str:>30s}  {r['f_mae']:8.2f}  {r['g_mae']:8.2f}  {r['total_mae']:8.2f}")

# Show detailed comparison for top 3
for i in range(min(3, len(results))):
    r = results[i]
    temps_str = ", ".join(f"{t:.1f}" for t in r['combo'])
    print(f"\n{'=' * 90}")
    print(f"RANK {i+1}: Temperatures = [{temps_str}]")
    print(f"{'=' * 90}")
    print(f"{'Strain':>12s}  {'f_calc':>8s}  {'f_exp':>8s}  {'f_err%':>8s}  "
          f"{'g_calc':>8s}  {'g_exp':>8s}  {'g_err%':>8s}")
    print("-" * 80)
    for j in range(len(exp_strain)):
        f_err = (r['f_avg'][j] - exp_f[j]) / exp_f[j] * 100
        g_err = (r['g_avg'][j] - exp_g[j]) / exp_g[j] * 100
        print(f"{exp_strain[j]:12.4e}  {r['f_avg'][j]:8.4f}  {exp_f[j]:8.4f}  {f_err:+8.2f}  "
              f"{r['g_avg'][j]:8.4f}  {exp_g[j]:8.4f}  {g_err:+8.2f}")
