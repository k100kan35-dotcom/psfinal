...
for r in range(1, len(temps) + 1):
    for combo in combinations(temps, r):
        ...
        try:
            ...
            f_results.append({'combo': combo, 'f_mae': f_mae, 'f_avg': f_avg})
        except:
            pass

f_results.sort(key=lambda x: x['f_mae'])
print(f"\n{'Rank':>4s}  {'Temperatures':>35s}  {'f_MAE%':>8s}")
print("-" * 60)
for i, r in enumerate(f_results):
    temps_str = ", ".join(f"{t:.2f}" for t in r['combo'])
    print(f"{i+1:4d}  {temps_str:>35s}  {r['f_mae']:8.2f}")

# ── 2) g만 독립 최적화 ──────────────────────────
print("\n" + "=" * 100)
print("g만 독립적으로 최적 온도 조합 탐색")
print("=" * 100)
...
for r in range(1, len(temps) + 1):
    for combo in combinations(temps, r):
        ...
        try:
            ...
            g_results.append({'combo': combo, 'g_mae': g_mae, 'g_avg': g_avg})
        except:
            pass

g_results.sort(key=lambda x: x['g_mae'])
print(f"\n{'Rank':>4s}  {'Temperatures':>35s}  {'g_MAE%':>8s}")
print("-" * 60)
for i, r in enumerate(g_results):
    temps_str = ", ".join(f"{t:.2f}" for t in r['combo'])
    print(f"{i+1:4d}  {temps_str:>35s}  {r['g_mae']:8.2f}")
...
print(f"\n{'=' * 100}")
print(f"최적 독립 조합:")
print(f"  f: [{', '.join(f'{t:.2f}' for t in best_f['combo'])}]  MAE={best_f['f_mae']:.2f}%")
print(f"  g: [{', '.join(f'{t:.2f}' for t in best_g['combo'])}]  MAE={best_g['g_mae']:.2f}%")
print(f"  합산 MAE = {(best_f['f_mae'] + best_g['g_mae'])/2:.2f}%")
print(f"{'=' * 100}")

print(f"\n{'Strain':>12s}  {'f_calc':>8s}  {'f_exp':>8s}  {'f_err%':>8s}  "
...
for i, s in enumerate(exp_strain):
    ...
    for T in temps:
        ...
        g_val = res['g_avg'][0] if res else float('nan')
        row += f"  {g_val:8.4f}"
    row += f"  {exp_g[i]:8.3f}"
    print(row)
...
# ── 5) 가중 평균 시도 (g에 대해) ────────────────
print(f"\n{'=' * 100}")
print("g: 가중 평균 탐색 (2개 온도, 다양한 비율)")
print("=" * 100)

# 모든 2개 온도 조합에 대해 가중 평균 테스트
from itertools import combinations as combs
weight_results = []
for t1, t2 in combs(temps, 2):
    for w1 in np.arange(0.0, 1.05, 0.05):
        w2 = 1.0 - w1
        res1 = average_fg_curves(fg_by_T, [t1], exp_strain, interp_kind='loglog_linear',
                                 avg_mode='mean', clip_leq_1=False)
        res2 = average_fg_curves(fg_by_T, [t2], exp_strain, interp_kind='loglog_linear',
                                 avg_mode='mean', clip_leq_1=False)
        if res1 is None or res2 is None:
            continue
        g_weighted = w1 * res1['g_avg'] + w2 * res2['g_avg']
        g_mae = np.nanmean(np.abs(g_weighted - exp_g) / np.where(exp_g > 0, exp_g, 1) * 100)
        weight_results.append({
            'temps': (t1, t2), 'weights': (w1, w2),
            'g_mae': g_mae, 'g_avg': g_weighted
        })

weight_results.sort(key=lambda x: x['g_mae'])
print(f"\n{'Rank':>4s}  {'T1':>6s}  {'T2':>6s}  {'w1':>5s}  {'w2':>5s}  {'g_MAE%':>8s}")
print("-" * 50)
for i in range(min(15, len(weight_results))):
    r = weight_results[i]
    print(f"{i+1:4d}  {r['temps'][0]:6.2f}  {r['temps'][1]:6.2f}  "
          f"{r['weights'][0]:5.2f}  {r['weights'][1]:5.2f}  {r['g_mae']:8.2f}")

# 마찬가지로 f에 대해서도
print(f"\n{'=' * 100}")
print("f: 가중 평균 탐색 (2개 온도, 다양한 비율)")
print("=" * 100)
...
for t1, t2 in combs(temps, 2):
    for w1 in np.arange(0.0, 1.05, 0.05):
        ...
        if res1 is None or res2 is None:
            continue
        f_weighted = w1 * res1['f_avg'] + w2 * res2['f_avg']
        f_mae = np.nanmean(np.abs(f_weighted - exp_f) / np.where(exp_f > 0, exp_f, 1) * 100)
        f_weight_results.append({
            'temps': (t1, t2), 'weights': (w1, w2),
            'f_mae': f_mae, 'f_avg': f_weighted
        })

f_weight_results.sort(key=lambda x: x['f_mae'])
print(f"\n{'Rank':>4s}  {'T1':>6s}  {'T2':>6s}  {'w1':>5s}  {'w2':>5s}  {'f_MAE%':>8s}")
print("-" * 50)
for i in range(min(15, len(f_weight_results))):
    r = f_weight_results[i]
    print(f"{i+1:4d}  {r['temps'][0]:6.2f}  {r['temps'][1]:6.2f}  "
          f"{r['weights'][0]:5.2f}  {r['weights'][1]:5.2f}  {r['f_mae']:8.2f}")

# ── 6) 최적 독립 가중 조합 상세 ─────────────────
best_fw = f_weight_results[0]
best_gw = weight_results[0]
print(f"\n{'=' * 100}")
print("최적 독립 가중 조합 상세:")
print(f"  f: T={best_fw['temps']}, w={best_fw['weights']}, MAE={best_fw['f_mae']:.2f}%")
print(f"  g: T={best_gw['temps']}, w={best_gw['weights']}, MAE={best_gw['g_mae']:.2f}%")
print(f"  합산 MAE = {(best_fw['f_mae'] + best_gw['g_mae'])/2:.2f}%")
print(f"{'=' * 100}")
